...
// Mock comments data for comics
const initialComicComments: Comment[] = [
  {
    id: '1',
    userId: 'user1',
    username: 'comic_reader',
    avatarUrl: null,
    content: 'Невероятная прорисовка! Каждая панель - произведение искусства.',
  ...
  },
  {
    id: '2',
    userId: 'user2',
    username: 'graphic_novel_fan',
    avatarUrl: null,
    content: 'Сюжет развивается очень динамично. Не могу дождаться следующего выпуска!',
...
const ComicComments: React.FC<ComicCommentsProps> = ({ comicId, authorId }) => {
  const { t, language } = useLanguage();
  const { user, canCommentOnBook } = useAuth();
  const { toast } = useToast();
  const [comments, setComments] = useState<Comment[]>(initialComicComments);
  const [newComment, setNewComment] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [commentsEnabled, setCommentsEnabled] = useState(true);
  const [editingCommentId, setEditingCommentId] = useState<string | null>(null);
  const [editingContent, setEditingContent] = useState('');
  const isComicAuthor = user?.id === authorId;

  useEffect(() => {
    // Check if comments are enabled for this comic
    if (user) {
      const enabled = canCommentOnBook(comicId, authorId);
      setCommentsEnabled(enabled);
    }
  }, [comicId, authorId, user, canCommentOnBook]);
                  ...
                  <div className="flex-1">
                    ...
                    {user && editingCommentId !== comment.id && (
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        {/* Heart button (only for comic author) */}
                        {isComicAuthor && (
                          <div className="flex items-center">
                            <Toggle
                              size="sm"
