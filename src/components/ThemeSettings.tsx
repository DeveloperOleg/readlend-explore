...
const ThemeSettings: React.FC = () => {
  const { baseTheme, uiStyle, toggleUIStyle, setBaseTheme } = useTheme();
  const { t } = useLanguage();

  const handleThemeChange = (value: string) => {
    setBaseTheme(value as 'light' | 'dark' | 'dark-night' | 'system');
  };
        ...
        >
          ...
          <div className="flex items-center space-x-3">
            ...
            <Label htmlFor="system" className="flex-1 cursor-pointer">
              <div className="flex items-center gap-4">
                <div className="w-16 h-12 bg-gradient-to-br from-white to-gray-800 border-2 border-gray-400 rounded-lg shadow-sm relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-full h-3 bg-gradient-to-r from-gray-100 to-gray-700"></div>
                  <div className="absolute top-3 left-1 w-10 h-2 bg-gradient-to-r from-gray-200 to-gray-600 rounded"></div>
                  <div className="absolute top-6 left-1 w-8 h-1 bg-gradient-to-r from-gray-300 to-gray-500 rounded"></div>
                  <div className="absolute top-8 left-1 w-12 h-1 bg-gradient-to-r from-gray-300 to-gray-500 rounded"></div>
                </div>
                <div>
                  <p className="font-medium">{t('settings.systemTheme')}</p>
                  <p className="text-sm text-muted-foreground">
