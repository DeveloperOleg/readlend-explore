  ...
  return (
    <div className="space-y-4">
      <div className="bg-card p-5 rounded-xl border border-border/50 space-y-3">
        <Label htmlFor="title" className="text-sm font-medium text-foreground">{t('publish.bookTitle')}</Label>
        <Input 
          id="title" 
          value={title} 
      ...
      <div className="bg-card p-5 rounded-xl border border-border/50 space-y-3">
        <Label htmlFor="authors" className="text-sm font-medium text-foreground">{t('publish.author')}</Label>
        <Input 
          id="authors" 
          value={authors} 
      ...
      <div className="bg-card p-5 rounded-xl border border-border/50 space-y-3">
        <Label htmlFor="genre" className="text-sm font-medium text-foreground">Жанр</Label>
        <Select value={genre} onValueChange={(value) => setGenre(value as BookGenre)}>
          <SelectTrigger id="genre" className="h-12 rounded-xl border-2 focus:border-primary transition-colors">
            <SelectValue placeholder="Выберите жанр" />
          </SelectTrigger>
          <SelectContent className="rounded-xl bg-popover border-border z-50">
      ...
      <div className="bg-card p-5 rounded-xl border border-border/50 space-y-3">
        <Label htmlFor="status" className="text-sm font-medium text-foreground">Статус книги</Label>
        <Select value={status} onValueChange={(value) => setStatus(value as BookStatus)}>
          <SelectTrigger id="status" className="h-12 rounded-xl border-2 focus:border-primary transition-colors">
      ...
      <div className="bg-card p-5 rounded-xl border border-border/50 space-y-3">
        <Label className="text-sm font-medium text-foreground">{t('publish.releaseDate')}</Label>
        <div className="flex gap-3">
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                className={cn(
                  "flex-1 h-12 justify-start text-left font-normal rounded-xl border-2 transition-colors",
            ...
            <PopoverContent className="w-auto p-0 rounded-xl bg-popover border-border z-50" align="start">
              <Calendar
                mode="single"
          ...
          <Button 
            ...
            className="h-12 px-6 whitespace-nowrap rounded-xl border-2 transition-colors"
          >
            Сегодня
          </Button>
        </div>
