...
const BookReader: React.FC = () => {
  ...
  // Добавляем функционал для возврата назад
  const handleBack = () => {
    navigate(-1);
  };
      ...
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-sm border-b">
        <div className="flex items-center justify-between p-4">
          ...
          <Button variant="ghost" size="icon">
            <span className="sr-only">{t('book.options') || 'Опции'}</span>
      ...
      <div className="flex flex-col items-center p-6 mb-4 border-b">
        ...
        <div className="w-full mt-6 flex justify-between">
          <Button variant="outline" className="flex-1 mr-2">
            ...
            {t('book.addToLibrary') || 'Добавить в библиотеку'}
          </Button>
          <Button className="flex-1 ml-2 bg-orange-500 hover:bg-orange-600">
            ...
            {t('book.read') || 'Читать'} 0/{totalPages}
          </Button>
        </div>
        ...
        <div className="w-full flex justify-between text-sm text-center">
          <button className="flex-1">{t('book.aboutTitle') || 'О тайтле'}</button>
          <button className="flex-1 font-medium border-b-2 border-primary">
            {t('book.chapters') || 'Главы'} {totalPages}
          </button>
          <button className="flex-1">{t('book.comments') || 'Комментарии'}</button>
        </div>
      </div>
